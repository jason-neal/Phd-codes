...
def ra2deg(ra):
    split = ra.split(":")
    deg = float(split[0])*360/24 + (float(split[1]) + (float(split[2])/60) )/60 
    return deg
...
#pyasl.baryCorr(jd, ra, dec, deq=0.0)
...
#pyasl.helcorr(obs_long, obs_lat, obs_alt, ra2000, dec2000, jd, debug=False)
#This function calculates the motion of an observer in the direction of a star

print("Tapas_hellcorr", tapas_helcorr)
...
plt.plot(wlprime1, NoBerv_trans, "r.-", label="Tapas Values correction")

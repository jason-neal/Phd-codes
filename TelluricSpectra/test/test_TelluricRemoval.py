...
def test_divide_spectra():
    ...
    assert all(
        divide_spectra(np.array([1, 2, 1, 2, 3, 1]), np.array([1, 2, 1, 2, 3, 1]))
        == np.array(np.array([1, 1, 1, 1, 1, 1]))
    )
...
def test_match_wl():
    ...
    assert all(
        match_wl(np.array(a), np.array(b), np.array(c))
...
def test_airmass_scaling():
    ...
    assert all(
        airmass_scaling(np.array([3, 4, 5]), 5, 4) == np.array([3, 4, 5]) ** (4 / 5)

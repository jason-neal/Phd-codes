...
def get_coords(
    ...
    while True:
        ...
        coords = fig.ginput(
            n=0, timeout=0, show_clicks=True, mouse_add=1, mouse_pop=2, mouse_stop=3
        )  # better way to do it
        plt.close()

        coords = sorted(
            coords, key=lambda x: x[0]
        )  # Sort Coords by x position ascending.
        return coords
...
def do_fit(wl, spec, init_params, stel=None, tell=None):
    """Perform fit using opt.curvefit and return the found parameters.
    ...
    if (stel is not None) & (tell is not None):
        ...
        params, __ = opt.curve_fit(
            lambda x, *params: func_with_stellar_and_tell(x, stelnum, tellnum, params),
            wl,
            spec,
            init_params,
        )
        ...
        params, __ = opt.curve_fit(
            lambda x, *params: func_with_stellar(x, stelnum, params),
            wl,
            spec,
            init_params,
        )
...
def adv_wavelength_fitting(
    ...
    wl_a = np.asarray(wl_a)  # make sure all are numpy arrays
    spec_a = np.asarray(spec_a)  # make sure all are numpy arrays
    wl_b = np.asarray(wl_b)  # make sure all are numpy arrays
    spec_b = np.asarray(spec_b)  # make sure all are numpy arrays
    delta_a = np.abs(np.mean(np.diff(wl_a)))  # average wl step
    delta_b = np.abs(np.mean(np.diff(wl_b)))

    assert len(AxCoords) is len(
        BxCoords
    ), "Length of Coords do not match {}, {}".format(len(AxCoords), len(BxCoords))
    for i in range(len(AxCoords)):
        ...
        while True:  # Was this a good fit
            try:  # RuntimeError of fitting
                ...
                a_coords = get_coords(
                    ...
                    text="Choose lines for calibration",
                    model=model,
                    ref=ref,
                )  # (x, y)'s
                b_coords = get_coords(
                ...
                if stel in ["y", "Yes", "YES" "yes"]:
                    ...
                    extra_telluric_lines = get_coords(
                        wl_a_sec,
                        sect_a,
                        wl_b_sec,
                        sect_b,
                    ...
                    extra_telluric_params = coords2gaussian_params(
                        extra_telluric_lines, delta_a
                    )
                ...
                if tell in ["y", "Yes", "YES" "yes"]:
                    ...
                    fit_params_b = do_fit(
                        wl_b_sec, sect_b, init_params_b, tell=telluric_params
                    )
                else:
                    num_telluric = 0
                ...
                cont_try = inputer(
                    "Would you like to keep trying to fit to this section Y/n?"
                )
            ...
            cal_params_a, __ = split_telluric_stellar(
                fit_params_a, num_stellar + num_extra
            )
            cal_params_b, __ = split_telluric_stellar(fit_params_b, num_telluric)
            fig, __, __ = overlay_fitting(
                wl_a_sec,
                sect_a,
                wl_b_sec,
                sect_b,
                show_plot=True,
                paramsA=fit_params_a,
                paramsB=fit_params_b,
                hor=1,
                init_params_a=cal_params_a,
        ...
        if fit_worked:
            ...
            fit_line_params_a, fit_stell_params, fit_extra_params = split_telluric_stellar_telluric(
                fit_params_a, num_stellar, num_extra
            )

            fit_line_params_b, fit_tell_params = split_telluric_stellar(
                fit_params_b, num_telluric
            )
            ...
            fig, __, __ = plot_both_fits(
                wl_a_sec,
                sect_a,
                wl_b_sec,
                sect_b,
                show_plot=True,
                title="Pick Results to use",
                fitcoords_a=fitted_coords_a,
                best_a=best_a_coords,
                fitcoords_b=fitted_coords_b,
            ...
            for i in range(0, len(fitted_coords_a)):
                ...
                while True:
                    include = raw_input(
                        "Use Peak # {} ".format(i + 1)
                        + "corresponding to"
                        + " [a-({0:.2f}, {1:.2f}), b-({2:.2f}, {3:.2f})]? y/N?".format(
...
def func_with_stellar_and_tell(x, num_stell: int, num_tell: int, *params):
    ...
    debug(pv("params[0]"))
    par = params[0]
    line_params, stellar_params, telluric_params = split_telluric_stellar_telluric(
        par, num_stell, num_tell
    )
    ...
    for i in range(0, len(telluric_params), 3):
        tell_ctr = telluric_params[i]
        tell_amp = abs(telluric_params[i + 1])  # always positive so peaks are downward
        tell_wid = telluric_params[i + 2]
        y_tell = y_tell - tell_amp * np.exp(
            -0.5 * ((x - tell_ctr) / tell_wid) ** 2
        )  # Add telluric lines
    ...
    y_combined = (
        y_line + y_tell
    ) * y_stel  # multiplication of stellar and telluric lines
    y_combined[y_combined < 0] = 0  # limit minimum value to zero
    return y_combined
...
#                                                                     #
#######################################################################
def split_telluric_stellar(
    params: List[float], num_stellar: int
    ...
    assert (
        3 * num_stellar <= par_len
    ), "There is more stellar lines '{}' than number of parameters '{}'".format(
        num_stellar, par_len
    )

    first_stel = (
        -3 * num_stellar if num_stellar != 0 else par_len
    )  # index of first stellar line (from end)
...
def split_telluric_stellar_telluric(
    params: List[float], num_stellar: int, num_telluric: int
    ...
    assert (
        3 * (num_stellar + num_telluric) <= par_len
    ), "Number of stellar and telluric lines > length of parameters."

    first_tell = (
        int(-3 * num_telluric) if num_telluric != 0 else par_len
    )  # index of first telluric line

    first_stel = (
        int(-3 * num_stellar + first_tell) if num_stellar != 0 else first_tell
    )  # index of first stellar line (from end)
    ...
    assert len(stellar_params) % 3 is 0, "Stellar line list not correct length"
    assert (
        sum([len(line_params), len(stellar_params), len(telluric_params)]) == par_len
    ), "Parameter split length error"
...
def params2coords(params):
    ...
    (x, y, sigma) tuples of peak coordinates
...
def plot_both_fits(
    ...
    if fita and fitb:
        assert len(fitcoords_a) is len(
            fitcoords_b
        ), "Coords not same length. {}, {}".format(len(fitcoords_a), len(fitcoords_b))
...
def overlay_fitting(
    ...
    if init_params_a is not None:
        ax1.plot(
            wl_a,
            func_for_plotting(wl_a, init_params_a),
            "--",
    ...
    if init_params_b is not None:
        ax2.plot(
            wl_b,
            func_for_plotting(wl_b, init_params_b),
            "--",
            label="Calibration lines.",
        )
...
def print_fit_instructions():
    ...
    instructions = """\nYou will be shown an overlay of the observed specrtra and
        ...
        The goal of this is to select absorption lines that are common
...
if __name__ == "__main__":
    ...
    Calibdata = IOmodule.read_2col(
        path + "Telluric_spectra_CRIRES_Chip-" + str(1) + ".txt"
    )
    # print(Calibdata)
    # get_rough_peaks()
    ...
    good_coords_a, good_coords_b = adv_wavelength_fitting(
        UnCalibdata[0], UnCalibdata[1], CoordsA, Calibdata[0], Calibdata[1], CoordsB
    )

...
base_dir = os.path.join(
    "/", "home", "jneal", "Phd", "data", "Crires", "BDs-DRACS", "2017"
)
...
def main(fname, apply_berv=False, export=False, show=False):
    ...
    if apply_berv:
        wlprime_obs, __ = barycorr_crires(
            observation.xaxis, observation.flux, observation.header
        )
        wlprime_tell, __ = barycorr_crires(
            tell_spec.xaxis, tell_spec.flux, observation.header
        )
    ...
    masked_obs = Spectrum(
        xaxis=observation.xaxis[tell_mask], flux=observation.flux[tell_mask]
    )
    masked_tell = Spectrum(
        xaxis=tell_spec.xaxis[tell_mask], flux=tell_spec.flux[tell_mask]
    )

    maskedin_obs = Spectrum(
        xaxis=observation.xaxis[~tell_mask], flux=observation.flux[~tell_mask]
    )
    maskedin_tell = Spectrum(
        xaxis=tell_spec.xaxis[~tell_mask], flux=tell_spec.flux[~tell_mask]
    )
    ...
    if show:
        ...
        plt.hlines(
            1 - mask_value, tell_spec.xaxis[0], tell_spec.xaxis[-1], linestyles="--"
        )
        plt.legend()
    ...
    if export:
        ...
        new_export_correction_2fits(
            ...
            [
                True,
                (mask_value, "telluric line depth limit"),
